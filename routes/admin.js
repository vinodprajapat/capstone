...
router.get('/quiz', function(req, res, next) {
  quiz.find({},{_id:0, comments:0},function(err, quizdetail){
      if(err){
          res.send(err);
      }
...
router.post('/addcomment', (req, res )=>{ 
    ...
    quiz.findOneAndUpdate({id: req.body.quizid  }, {$push: {comments: comment }}, function(err, doc){
        if(err){
            console.log("Something wrong when updating data!" + err);
        }
    
        res.json("comments from server after addition " + doc );
    });
})


router.get('/comments/:id', function(req, res, next) {
    quiz.find({id: req.params.id},{comments:1},function(err, comments){
