  ...
  onSubmit() {
   ...
   goBack(): void {
    ...
    showComment(){
      ...
      this.quizService.getComment(this.quiz.id).subscribe(res=>{
       this.cmts=res;
      this.comments=this.cmts[0].comments;
    ...
    addComment(){
      var comment={
        quizid:this.quiz.id,
        user:this.username,
        cmt:this.usercomment
      }
      this.quizService.addComment(comment).subscribe((res)=>{ 
          //   this.cmts=res;
          // this.comments=this.cmts[0].comments;
         console.log(res);
